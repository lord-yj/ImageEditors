...
public class ScissorsSelectionModel extends SelectionModel {
    ...
    protected void startSelection(Point start) {
        ...
        // Find the shortest paths from our start node,so we will know what segment to connect to the
    ...
    protected void appendToSelection(Point p) {
        ...
        PolyLine constructed = graph.pathToPolyLine(path);
        //Append to the selection
        selection.add(constructed);
        //Find the new shortest-path from the end point of the selection
        findPaths(vertexId);
    ...
    public PolyLine liveWire(Point p) {
        int vertex = graph.idAt(p);
        //Construct the shortest path to the vertex
        List<Integer> pathSoFar = paths.pathTo(vertex);
        //Return it
        return graph.pathToPolyLine(pathSoFar);
    ...
    private class ShortestPathsWorker
        ...
        public PathfindingSnapshot doInBackground() {
            ...
            //Initialize to null, so that it could be updated in the loop
            PathfindingSnapshot pathsSoFar = null;
            while (!pathfinder.allPathsFound() && !isCancelled()) {
                //Get the paths so far
                pathsSoFar = pathfinder.extendSearch(7000);
                //Come back to it
                int increment = (int) (100.0 * pathfinder.settledCount()
                        / pathfinder.vertexCount());
                //Set the progress
                setProgress(increment);
                //Publish the result so far
                publish(pathsSoFar);
                //If cancelled
                if (isCancelled()) {
                    return pathsSoFar;
