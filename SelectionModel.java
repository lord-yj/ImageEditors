...
public abstract class SelectionModel {
    ...
    public Point lastPoint() {
        if (state == NO_SELECTION) {
            throw new IllegalStateException(
                    "Cannot query last point when no selection has been started");
        }
        if (selection.isEmpty()) {
            return start;
        } else {
            PolyLine current = selection.getLast();
            return current.end();
        }
    }
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        Point minStart = this.selection.get(0).start();
        int initialDistance = (int) (Math.pow(minStart.x - p.x, 2) + Math.pow(minStart.y - p.y, 2));
        ListIterator<PolyLine> iterator = this.selection.listIterator();
        int closestIndex = 0;
        int index = 0;
        while (iterator.hasNext()) {
            PolyLine current = iterator.next();
            int currentDistance = (int) (Math.pow(current.start().x - p.x, 2) + Math.pow(
                    current.start().y - p.y, 2));
        ...
        if (initialDistance > maxDistanceSq) {
            return -1;
        }
        return closestIndex;
    ...
    protected void undoPoint() {
        if (selection.isEmpty()) {
            // If there are no segments, reset to clear the starting point and ensure the state is NO_SELECTION
            reset();
        } else {
            // Capture the last segment to remove
            PolyLine toRemove = selection.getLast();
            // Create a copy of the current selection for the 'old' state in notification
            List<PolyLine> oldSelection = new ArrayList<>(selection);
            SelectionState oldState = state;

            // Remove the last segment
            selection.remove(toRemove);

            // Check if there are no more segments left
            if (selection.isEmpty()) {
                if (state != NO_SELECTION) {
                    state = NO_SELECTION;
                    propSupport.firePropertyChange("state", oldState, state);
            ...
            propSupport.firePropertyChange("selection", oldSelection, new ArrayList<>(selection));
        }
    }
